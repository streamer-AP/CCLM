...
if __name__ == "__main__":
    ...
    fea = torch.zeros_like(x)
    # inputs = {"predict_counting_map": x, "offset_map": torch.zeros(1, 2, 6, 6).cuda()}
    inputs = {"predict_counting_map": x, "offset_map": torch.zeros(1, 2, 1024, 1024).cuda()}
    print(x)
    x.requires_grad = True
    ...
    off = torch.zeros(1, 2, 1024, 1024).cuda()
    ...
    for i in range(10):
        ...
        st = time.time()
        lossall = loss(inputs, targets)["all"]
        end = time.time()
        print(end -st)
        (grad_x, grad_off) = torch.autograd.grad(lossall, (x, off))
        # print(grad_x.shape)
