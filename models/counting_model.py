...
class SingleScaleEncoderDecoderCounting(nn.Module):
    def __init__(self, args) -> None:
        ...
        if "stride" in args:
            ...
            self.stride=2
        # self.get_model_complexity(input_shape=(3, 1024,1024))
        # self.backbone.cuda()
        # self.decoder_layers.cuda()
        # with torch.no_grad():
        #     test_input = torch.randn(1, 3, 5120, 5120).cuda().half()
...
class DoubleEncoderDecoderCounting(nn.Module):
    ...
    def __init__(self, args) -> None:
        ...
        # self.get_model_complexity(input_shape=(3, 5120,5120))
    @autocast("cuda")
    def forward(self, x, ext_info=None):
        print("x")
        print(x.shape)
