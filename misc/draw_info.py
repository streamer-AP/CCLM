        ...
        if self.click():
            ...
            if self.draw_original and inputs is not None:
                for j in range(inputs.shape[0]):
                    ...
                    origin_input = (origin_input.detach().cpu().numpy()*255).astype(np.uint8)
                    # plt.imshow(origin_input)
                    # plt.savefig(os.path.join(sub_dir, f"{j}.png"))
                    # plt.close()
                    # print("save_path", os.path.join(sub_dir, f"{header}_input_{self.cnt}_{j}.jpg"))
            ...
            #     for j in range(len(out_dict["masks"])):
            #         mask = out_dict["masks"][j].detach().cpu()
            ...
            #         plt.figure(figsize=(10, 10))
            #         plt.imshow(mask)
            #         plt.savefig(os.path.join(sub_dir, f"{header}_mask_{self.cnt}_{j}.jpg"))
            #         plt.close()
